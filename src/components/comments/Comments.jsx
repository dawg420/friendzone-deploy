...
export default function Comments(props) {
  ...
  useEffect(() => {
    const testFunction = async () => {
      ...
      const postComments = await getComments(props.comments);
      var commentsArray = [];
      postComments.forEach((doc) => commentsArray.push(doc));

      for (const doc of commentsArray) {
        const userInfo = await getUserInfo(doc.data().user);
        const IsLiked = thisUserInfo.likedComments.includes(doc.data().id);
        let props = {
          postId: postId,
          ...
          dateCreated: getDate(doc.data().dateCreated.toDate()),
          body: doc.data().body,
