...
export async function addUser(displayName, email) {
  try {
    ...
    await addDoc(collection(firestore, "users", email, "notifications"), {});
    await addDoc(collection(firestore, "users", email, "unreadCount"), {});
...
export async function getComments(commentsId) {
  try {
